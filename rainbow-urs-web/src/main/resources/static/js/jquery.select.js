...
(function ($) {
    ...
    $.bdxSelect = function (element, settings) {
        if (settings.init) {
            ...
            $.getJSON(settings.url, {dict: settings.dict, pValue: settings.parentValue}, function (json) {
                var _html = $.buildContent(json, settings.selected);
    ...
    $.buildContent = function (data, selected) {
        var _html = '';
        $.each(data, function (i, v) {
            _html += '<option value="' + v.pValue + '" ' + (selected != undefined && selected != null && selected != "" && selected === v.pValue ? "selected" : "") + '>' + v.pDesc + '</option>';
        });
        return _html;
    };
