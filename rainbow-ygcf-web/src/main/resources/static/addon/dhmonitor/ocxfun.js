...
function  initPlay(channelId){
	  if(channelId!=null&&channelId!=''){
		  ButtonStartRealplayByWndNo(channelId,0);  
	  }else{
	    var winnum=parseInt($("#textWndNum").val());
	    var treeObj = $.fn.zTree.getZTreeObj("ztree");
	    var treeNode = treeObj.transformToArray(treeObj.getNodes());
	    var playwin=winnum;
	    if(treeNode.length<winnum){
	    	playwin=treeNode.length;
	    }
	    for(var i=0;i<playwin;i++){
	    	//alert(treeNode[i].deviceSn);
	    	//初始化窗口播放功能
	    	if(i==0){$("#textCameraID").val(treeNode[i+1].deviceSn);}
	    	ButtonStartRealplayByWndNo(treeNode[i+1].deviceSn,i);
	    	
	    }
	  }  
}

function  initPlayOne(channelId){
	    //初始化窗口播放功能
	    ButtonStartRealplayByWndNo(channelId,0);
}
...
function init(){
	...
	//var nWndNo = obj.DPSDK_GetSelWnd(gWndId);
	//ShowCallRetInfo(obj.DPSDK_DirectRealplayByWndNo(gWndId, nWndNo, "1000047$1$0$9", 1, 1, 1), "直接实时播放");
	for(var i=1;i<=4;i++)
...
function SetButtonCreateWnd_onclick(num) 
{
	var obj = document.getElementById("DPSDK_OCX");
    var nWndCount =num;
...
//登陆平台DPSDK_Login
function AutoButtonLogin_onclick(ip,port,username,pwd) 
{
	var obj = document.getElementById("DPSDK_OCX");
	ShowCallRetInfo(nRet, "登录中……");
	var nRet = obj.DPSDK_Login(ip, port, username,pwd);
...
function StartRealplayByWndNo() 
{
    var obj = document.getElementById("DPSDK_OCX");
    
    var szCameraId = document.getElementById("textCameraID").value;
    //var nStreamType = document.getElementById("selectStreamType").value;
    var nStreamType ='1';//主码流，辅码流
    //var nMediaType = document.getElementById("selectMediaType").value;
    var nMediaType ='1'; //视频
    //var nTransType = document.getElementById("selectTransType").value;
    var nTransType ='TCP';//1 TCP 0 UDP
	var nWndNo = obj.DPSDK_GetSelWnd(gWndId);
    ShowCallRetInfo(obj.DPSDK_StartRealplayByWndNo(gWndId, nWndNo, szCameraId, nStreamType, nMediaType, nTransType), "播放视频");
	if(obj.DPSDK_StartRealplayByWndNo(gWndId, nWndNo, szCameraId, nStreamType, nMediaType, nTransType) == 0)
	{
		insertRealplayHistory(szCameraId);
	}
}

function ButtonStartRealplayByWndNo(szCameraId,wndNo)
{
	var nWndNo=0;
	var obj = document.getElementById("DPSDK_OCX");
    //var szCameraId = document.getElementById("textCameraID").value;
...
function ButtonStartRealplayByHWND_onclick() 
{
	...
	if(obj.DPSDK_StartRealplayByHWND(hWnd, szCameraId, nStreamType, nMediaType, nTransType) == 0)
	{
		insertRealplayHistory(szCameraId);
	}
}
...
//根据当前选择的窗口抓图
function ButtonCapturePictureByWndNo_onclick(path)
{
	var obj = document.getElementById("DPSDK_OCX");
	var nWndNo = obj.DPSDK_GetSelWnd(gWndId);
	var mydate=new Date();
	...
	path=path.replace(/\\/g,"\\\\");
	path=path+mydate.toLocaleString().replace(" ","").replace("年","").replace("月","").replace("日","").replace(reg,"")+".bmp";
	//alert(path.toString());
	alert(obj.DPSDK_CapturePictureByWndNo(gWndId, nWndNo,path.toString())==0?'操作成功！':'操作失败！');

	//alert(obj.DPSDK_CapturePictureByWndNo(gWndId, nWndNo,path.toString()));
...
function ButtonStartTimePlaybackByWndNo_onclick(strStartTime, strEndTime)
{
	var obj = document.getElementById("DPSDK_OCX");
 ...
 //   var strEndTime = document.getElementById("textEndTime").value;
    var nStartTime = getDate(strStartTime).getTime()/1000;
	var nEndTime = getDate(strEndTime).getTime()/1000;
	var result=obj.DPSDK_StartTimePlaybackByWndNo(gWndId, nWndNo, szCameraId, nRecordSource, nStartTime, nEndTime);
	alert(result==0?'获取成功！':'获取失败！');
