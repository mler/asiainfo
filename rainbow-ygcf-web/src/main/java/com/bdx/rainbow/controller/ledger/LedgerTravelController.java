 ...
 */
@Controller
@RequestMapping("ledgerTravel")
public class LedgerTravelController extends BaseController {
    private static final Logger logger = LoggerFactory.getLogger(LedgerTravelController.class);

    // vm文件存放根目录
    private static final String VM_ROOT_PATH = "sk/ledger/travel/%s";
    ...
    public ModelAndView detailList(Model model, PageInfo pageInfo, LedgerTravel ledgerTravel) {
    ...
    public ModelAndView ticketList(Model model, LedgerTravel ledgerTravel) {
        try {
            Map<String, String> map = new HashMap<>();
            // 采购明细ID不为空
            if(StringUtil.isNotEmptyObject(ledgerTravel.getTravelId())) {
                map = ledgerTravelService.ticketSingle(ledgerTravel.getTravelId());

                ledgerTravel = ledgerTravelService.get(ledgerTravel.getTravelId());
            }
            // 台账ID不为空
            else if(StringUtil.isNotEmptyObject(ledgerTravel.getLedgerId())) {
                map = ledgerTravelService.ticketList(ledgerTravel.getLedgerId());
            }

            model.addAttribute("map", map);
            model.addAttribute("ledgerId", ledgerTravel.getLedgerId());
    ...
    public ModelAndView detailDetailList(Model model, PageInfo pageInfo, LedgerTravel ledgerTravel) {
