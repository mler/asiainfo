 ...
 */
@Controller
@RequestMapping("ledgerSterilize")
public class LedgerSterilizeController extends BaseController {
    private static final Logger logger = LoggerFactory.getLogger(LedgerSterilizeController.class);

    // vm文件存放根目录
    private static final String VM_ROOT_PATH = "sk/ledger/sterilize/%s";
    ...
    public ModelAndView detailList(Model model, PageInfo pageInfo, LedgerSterilize ledgerSterilize) {
    ...
    public ModelAndView ticketList(Model model, LedgerSterilize ledgerSterilize) {
        try {
            Map<String, String> map = new HashMap<>();
            // 采购明细ID不为空
            if(StringUtil.isNotEmptyObject(ledgerSterilize.getSterilizeId())) {
                map = ledgerSterilizeService.ticketSingle(ledgerSterilize.getSterilizeId());

                ledgerSterilize = ledgerSterilizeService.get(ledgerSterilize.getSterilizeId());
            }
            // 台账ID不为空
            else if(StringUtil.isNotEmptyObject(ledgerSterilize.getLedgerId())) {
                map = ledgerSterilizeService.ticketList(ledgerSterilize.getLedgerId());
            }

            model.addAttribute("map", map);
            model.addAttribute("ledgerId", ledgerSterilize.getLedgerId());
    ...
    public ModelAndView detailDetailList(Model model, PageInfo pageInfo, LedgerSterilize ledgerSterilize) {
