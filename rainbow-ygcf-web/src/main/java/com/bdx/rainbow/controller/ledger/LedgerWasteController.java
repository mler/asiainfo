 ...
 */
@Controller
@RequestMapping("ledgerWaste")
public class LedgerWasteController extends BaseController {
    private static final Logger logger = LoggerFactory.getLogger(LedgerWasteController.class);

    // vm文件存放根目录
    private static final String VM_ROOT_PATH = "sk/ledger/waste/%s";
    ...
    public ModelAndView detailList(Model model, PageInfo pageInfo, LedgerWaste ledgerWaste) {
    ...
    public ModelAndView ticketList(Model model, LedgerWaste ledgerWaste) {
        try {
            Map<String, String> map = new HashMap<>();
            // 采购明细ID不为空
            if(StringUtil.isNotEmptyObject(ledgerWaste.getWasteId())) {
                map = ledgerWasteService.ticketSingle(ledgerWaste.getWasteId());

                ledgerWaste = ledgerWasteService.get(ledgerWaste.getWasteId());
            }
            // 台账ID不为空
            else if(StringUtil.isNotEmptyObject(ledgerWaste.getLedgerId())) {
                map = ledgerWasteService.ticketList(ledgerWaste.getLedgerId());
            }

            model.addAttribute("map", map);
            model.addAttribute("ledgerId", ledgerWaste.getLedgerId());
    ...
    public ModelAndView detailDetailList(Model model, PageInfo pageInfo, LedgerWaste ledgerWaste) {
