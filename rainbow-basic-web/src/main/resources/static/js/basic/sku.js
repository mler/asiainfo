...
function querySku(){
	...
	var createName = $.trim($("#creater").val());
	var status = $("#status").val();
	var admin = $.trim($("#admin").val());
	if(admin == '' || admin == undefined || admin == null){
		location.href = base + '/sku/list?skuBarcode=' + skuBarcode + '&skuName=' + skuName + '&creater=' + createName + '&status=' + status;
	}else{
		location.href = base + '/sku/list?skuBarcode=' + skuBarcode + '&skuName=' + skuName + '&creater=' + createName + '&status=' + status + '&admin=' + admin;
	}
}