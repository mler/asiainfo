...
public class SeedTaskCache {
	...
	{
		for(int i=0;i<100;i++)
		SeedTaskCache.addSeed(new GdsSupplierSeed(""+i));
		
		for(int j=0;j<3;j++)
		{
			Collection<Seed> seeds = SeedTaskCache.getNext(GdsSupplierSeed.class, 5);
		
		for(Seed seed : seeds)
			System.out.println(JacksonUtils.toJson(seed));
		}
