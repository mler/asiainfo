...
//	public static AtomicLong 	good_success 	= new AtomicLong(0);//请求成功数
//
	/** 用于计算种子线程池中存货的线程 **/
...
//	
	 ...
	 * @param seed
	 */
	public synchronized void addTask(SeedTask seed)
	{
		completionService.submit(seed);
		thread_submit.addAndGet(1);
		try {
			logger.info("有新任务提交，总数["+thread_submit+"] seed："+JacksonUtils.toJson(seed));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
...
//				log.debug("["+thread_completed.get()+"],共"+(pages == null?"0":((Collection)pages).size())+"个 :"+JacksonUtils.toJson(pages));
	...
	public void registerScheduleTask(Runnable command,long time) throws Exception
	{
		...
		((ScheduledThreadPoolExecutor)executor).scheduleWithFixedDelay(command, 60, 10, TimeUnit.SECONDS);
	}
	...
	public void shutDown()
	{
		try{
			...
			logger.debug("Runnable 数量："+runs.size());
			
			while(executor.isTerminated() == false){  
				logger.debug("正在关闭线程....");  
	            Thread.sleep(1000);    
	        }  
			logger.debug("所有的子线程都结束了！");  
		}
		catch(InterruptedException e)
		{
			e.printStackTrace();
			logger.error("关闭失败，error:{}",e.getMessage());
		}
	}
