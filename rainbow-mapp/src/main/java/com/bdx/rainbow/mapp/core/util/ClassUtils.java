		...
		try {
			...
			while (dirs.hasMoreElements()) {
				...
				if ("file".equals(protocol)) {
					...
					logger.debug("jar类型的扫描:"+fileUrl);
					String path = fileUrl.substring(fileUrl.indexOf("file:")+5, fileUrl.length());
					String[] pathAndPackage = path.split("!");
					if(pathAndPackage[1].startsWith("/"))
						pathAndPackage[1] = pathAndPackage[1].replaceFirst("/", "");
					clazzs.addAll(getClasssFromJarFile(pathAndPackage[0], pathAndPackage[1]));
				}
			}
...
//				clazzs.add(Class.forName(className));
	...
	public static void main(String[] args) {
		...
		String fileUrl = "file:/Users/mler/work/repository/com/bdx/rainbow-ygcf-service/0.0.1-SNAPSHOT/rainbow-ygcf-service-0.0.1-SNAPSHOT.jar!/com/bdx/rainbow";
		logger.debug("jar类型的扫描:"+fileUrl);
		String path = fileUrl.substring(fileUrl.indexOf("file:")+5, fileUrl.length());
		String[] pathAndPackage = path.split("!");
		if(pathAndPackage[1].startsWith("/"))
			pathAndPackage[1] = pathAndPackage[1].replaceFirst("/", "");
//		Set<Class<?>> clazzs = ClassUtils.getClasssFromPackage("com.bdx.rainbow.mapp");//
//		for (Class<?> clazz : clazzs) {
//			logger.debug(clazz.getName());
//		}

		Set<Class<?>> clazzs = ClassUtils.getClasssFromJarFile(pathAndPackage[0],pathAndPackage[1]);
		 for (Class clazz : clazzs) {
			 logger.debug(clazz.getName());
		 }
//		
